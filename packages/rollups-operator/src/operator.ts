// modified from https://github.com/dot-i/k8s-operator-node
...
export class ResourceMetaImpl implements ResourceMeta {
    ...
    public static createWithPlural(
        plural: string,
        object: KubernetesObject,
    ): ResourceMeta {
...
export default abstract class Operator {
    ...
    private resourcePathBuilders: Record<
        string,
     ...
     */
    constructor(kubeConfig: KubeConfig, logger?: OperatorLogger) {
    ...
    protected async registerCustomResourceDefinition(crdFile: string): Promise<{
        ...
        try {
            ...
            if (
                ...
                throw new Error(
                    "Invalid CRD yaml (expected 'apiextensions.k8s.io')",
                );
            }
            ...
            this.logger.info(
                `registered custom resource definition '${crd.metadata?.name}'`,
            );
        } catch (err) {
                ...
                .watch(
                    ...
                    (phase, obj) =>
                        this.eventQueue.push({
                            event: {
                                meta: ResourceMetaImpl.createWithPlural(
                                    plural,
                                    obj,
                                ),
                                object: obj,
                    ...
                    (err) => {
                        if (err) {
                            this.logger.error(
                                `watch on resource ${id} failed: ${this.errorToJson(
                                    err,
                                )}`,
                            );
                            process.exit(1);
    ...
    protected async handleResourceFinalizer(
        ...
        if (
            ...
            if (
                metadata.finalizers &&
                metadata.finalizers.includes(finalizer)
            ) {
                ...
                const finalizers = metadata.finalizers.filter(
                    (f) => f !== finalizer,
                );
                await this.setResourceFinalizers(event.meta, finalizers);
            }
     ...
     */
    protected async setResourceFinalizers(
        meta: ResourceMeta,
        finalizers: string[],
    ): Promise<void> {
        ...
        await gaxios.request(options).catch((error: any) => {
            if (error) {
    ...
    protected async applyAxiosKubeConfigAuth(request: {
        ...
        if (opts.headers?.Authorization) {
            ...
            request.headers.Authorization = opts.headers
    ...
    protected async applyGaxiosKubeConfigAuth(options: {
        ...
        if (opts.headers?.Authorization) {
            ...
            options.headers.Authorization = opts.headers
                .Authorization as string;
        } else if (opts.auth) {
            options.headers = options.headers ?? {};
    ...
    private async resourceStatusRequest(
        ...
        const options: GaxiosOptions = {
            method,
            url:
                this.resourcePathBuilders[meta.id](meta) +
                `/${meta.name}/status`,
            data: body,
        };
        if (method === "PATCH") {
        ...
        try {
            ...
            return response
                ? ResourceMetaImpl.createWithId(meta.id, response.data)
                : null;
        } catch (err) {
